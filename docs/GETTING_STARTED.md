...
- **システムアーキテクチャを理解する**: [DOCKER_ECR_DEPLOYMENT_GUIDE.md](./DOCKER_ECR_DEPLOYMENT_GUIDE.md)
  - 2つのLambda関数が同じイメージを使う仕組み
  - Dockerイメージの内部構造
  - Terraformとの関係

- **CI/CDをテストする**: [CI_CD_TESTING_GUIDE.md](./CI_CD_TESTING_GUIDE.md)
  - 3つのテストシナリオ
  - GitHub Actionsの確認方法
  - トラブルシューティング

- **Terraformを深く理解する**: [Terraform/SETUP_GUIDE.md](../Terraform/SETUP_GUIDE.md)
  - モジュール構成
  - 変数の設定
  - リソースの詳細
...
1. **環境変数の追加**: `Terraform/aws/lambda_fastapi/main.tf` を編集
2. **新しいAPIエンドポイント追加**: `src/backend/main.py` を編集
3. **Lambdaメモリ・タイムアウト変更**: `Terraform/aws/variables.tf` を編集
4. **Step Functionsワークフロー変更**: `Terraform/aws/step_functions/definition.json.tpl` を編集
...
**セットアップで問題が発生した場合は、各ドキュメントのトラブルシューティングセクションを参照してください。**
