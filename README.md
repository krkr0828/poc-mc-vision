...
本PoCでは、AI推論基盤の中核となる部分（推論パイプライン、監視、基本的なセキュリティ設定）にフォーカスして実装・検証しました。本番環境では、要件に応じて次のような要素を追加することを想定しています。

- **認証・認可**: API Gateway + Cognito による利用者認証と、IAM ロールベースのアクセス制御
- **ネットワークセキュリティ**: VPC 内配置、VPC エンドポイント経由のサービス接続、WAF や DDoS 対策の適用
- **データ分析基盤**: 推論ログの S3 への保存と、Glue + Athena による分析環境の構築
- **MLOps 機能**: SageMaker Model Registry によるモデルバージョン管理や、A/B テストを前提とした段階的リリース
